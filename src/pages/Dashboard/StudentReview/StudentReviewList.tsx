...
const StudentProjectList: React.FC = () => {
  ...
  const { data, isLoading }: any = useGetStudentReviewsQuery("");
  const [deleteStudentReview] = useDeleteStudentReviewMutation();
  ...
  const confirmDelete = async (record: any) => {
    const { data }: any = await deleteStudentReview(record.id);
  ...
  return (
    <div>
      ...
      <div className="mb-3 d-flex justify-content-between">
        <h5 className="mb-0">Student Review List</h5>
        <Button
          type="default"
          className="d-flex align-items-center gap-1"
          onClick={() => navigate("/dashboard/student-reviews-create")}
        >
          <FaPlus /> Create
        </Button>
      </div>
